...
class Jogador:
    ...
    def mostrar_campo(self, titulo=None):
        ...
        for i, c in enumerate(self.campo, 1):
            ...
            print(f"  {i}. {c.name} | ATK: {c.atk_total()} | DEF: {c.def_total()}{status}")
    ...
    def turno_ia(self, oponente):
        ...
        print("\n--- FASE DE ATAQUE (IA) ---")
        for atacante in [c for c in self.campo if c.atk_total() > 0 and not c.ja_atacou]:
            if not oponente.vivo:
                break
...
if __name__ == "__main__":
    ...
    # Coloque seed=42 se quiser sempre o mesmo embaralhamento
    jogador = Jogador("Você", deck, rogen, is_ia=False, seed=None)
    ia = Jogador("Oponente IA", deck, rogen, is_ia=True, seed=None)
